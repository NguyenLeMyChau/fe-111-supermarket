...
import "./Receipt.scss";
import { getInvoiceById, getInvoiceRefundById } from "../../../services/cartRequest";
import ReceiptRefund from "./ReceiptRefund";

const PaymentModalRefund = ({ isPaid, closeModal, accessToken, axiosJWT, invoiceId }) => {
  ...
  useEffect(() => {
    if (isPaid && invoiceId) {
      const fetchInvoice = async () => {
        try {
          const invoiceData = await getInvoiceRefundById(accessToken, axiosJWT, invoiceId);     
  ...
  return (
    ...
    >
      ...
      <div id="receiptToPrint">
        {data ? <ReceiptRefund data={data} /> : <p>Loading...</p>}
      </div>
...
export default PaymentModalRefund;
